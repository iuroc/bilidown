...
1. 数据使用 SQLite 存储，具体存储操作以开发情况为准，比如涉及到频繁更新进度信息的操作等。
2. 对于软件的设置项，也保存到 SQLite 的一张 KV 表 `setting` 中，通过提供 `getSettings(): SettingsInterface` 方法来读取，并提供给前端使用。
3. 视频下载过程中，不允许修改下载路径，所以下载任务的保存路径取决于创建数据库记录的那一刻，且创建记录时默认读取软件设置中的路径。

### 关于登录验证

1. 登录信息保存在 SQLite 的 user 表，
